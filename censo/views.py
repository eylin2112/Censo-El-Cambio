...
class JefeFamiliarEditar(UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid() and form2.is_valid():
            ...
            if fecha_nacimiento > fecha_limite:
                messages.error(request, 'El jefe familiar no puede ser menor a 15.')
                return render(request, self.template_name, {'form': form, 'form2': form2,
                                                    'boton': 'Registrar jefe familiar', 'seccion': 'Registrar Jefe',
                                                    'titulo': 'Registrar Jefe Familiar'})
            habitante = form.save()
...
def HabitantesPDF(request):
  ...
  pdf = HTML(string=template.render(context), base_url=request.build_absolute_uri()).write_pdf(stylesheets=[CSS(css_url)])
